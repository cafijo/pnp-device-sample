...
public class Thermostat {
    ...
    private static final ConcurrentHashMap<Date, Double> temperatureReadings = new ConcurrentHashMap<>();
    ...
    public static void main(String[] args) throws Exception {
        ...
        log.debug("Set handler to receive direct method command.");
        deviceClient.subscribeToMethods(new DirectMethodCallback(), null);
    ...
    private static class DirectMethodCallback implements com.microsoft.azure.sdk.iot.device.twin.MethodCallback {
        ...
        final String resetTemperature = "resetTemperature";
        ...
        public DirectMethodResponse onMethodInvoked(String methodName, DirectMethodPayload methodData, Object context) {
            switch (methodName) {
                case getMaxMinReport:
                    return onGetMaxMinReport(methodData);
                case resetTemperature:
                    return onResetTemperature(methodData);
                case reboot:
                    return onReboot(methodData);
                default:
    ...
    public static DirectMethodResponse onResetTemperature(DirectMethodPayload methodData) {
        temperatureReset = true;
        return new DirectMethodResponse(StatusCode.COMPLETED.value, null);
    }

    public static DirectMethodResponse onReboot(DirectMethodPayload methodData) {
        //TODO: Reboot system
        return new DirectMethodResponse(StatusCode.COMPLETED.value, null);
    }

    public static DirectMethodResponse onGetMaxMinReport(DirectMethodPayload methodData) {
        String jsonRequest = methodData.getPayload(String.class);
        ZonedDateTime zonedDateTime = ZonedDateTime.now(ZoneOffset.UTC).minusMinutes(5);
